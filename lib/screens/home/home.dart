...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> getCount() async {
    int count = 0;
    count = await MainModel.getMachineCount(
        widget.dorm.toString(), widget.floor.toString(), 'setakkiCount');
    this.setakkiCount = count;
    count = await MainModel.getMachineCount(
        widget.dorm.toString(), widget.floor.toString(), 'gunjokiCount');
    this.gunjokiCount = count;
    setakkiAvailable = await OnboardingModel.getStateCount(
        widget.dorm.toString(), widget.floor.toString(), "setakki");
    gunjokiAvailable = await OnboardingModel.getStateCount(
        widget.dorm.toString(), widget.floor.toString(), "gunjoki");
    if (this.mounted) {
      setState(() {});
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
          padding:
              EdgeInsets.fromLTRB(width * 0.1, height * 0.01, width * 0.1, 0),
          child: ListView(
              children: <Widget>[
                Padding(
                  padding:
                ...
                Row(
                  ...
                  children: [
                    Text('세탁기', style: head2style(color: darkGrey)),
                    RichText(
                        text: TextSpan(children: <TextSpan>[
                      TextSpan(
                          text: '사용가능: ', style: subtitle2style(color: grey)),
                      TextSpan(
                          text: '$setakkiAvailable',
                          style: body1style(color: darkGrey)),
                ...
                Container(
                    ...
                    child: ListView.separated(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return Row(
                            children: [
                              BuildSetakki(widget.dorm, widget.floor, index + 1,
                                  '${index + 1}setakki'),
                ...
                Column(
                  ...
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('건조기', style: head2style(color: darkGrey)),
                        RichText(
                            text: TextSpan(children: <TextSpan>[
                          TextSpan(
                              text: '사용가능: ',
                          ...
                          TextSpan(
                              text: '$gunjokiAvailable',
                              style: body1style(color: darkGrey)),
                          TextSpan(
                              text: '대', style: subtitle2style(color: grey))
                        ])),
                      ],
                    ),
                    SizedBox(height: height * 0.01),
                    Container(
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return Row(
                              children: [
                                BuildGunjoki(widget.dorm, widget.floor,
                                    index + 1, '${index + 1}gunjoki'),
                                SizedBox(width: width * 0.05)
                              ],
                            );
                          }),
